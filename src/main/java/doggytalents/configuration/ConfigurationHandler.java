...
public class ConfigurationHandler {
    ...
    public static void loadConfig() {
        ...
        Constants.DOGS_IMMORTAL = CONFIG.get(
            CATEGORY_DOGGYSETTINGS,
            "isDogImmortal",
            true,
            ...
            .getBoolean(true);
        Constants.TEN_DAY_PUPS = CONFIG
            .get(CATEGORY_DOGGYSETTINGS, "tenDayPuppies", true, "Determines if pups take 10 days to mature.")
            .getBoolean(true);
        Constants.IS_HUNGER_ON = CONFIG
            .get(CATEGORY_DOGGYSETTINGS, "isHungerOn", true, "Enables hunger mode for the dog")
            .getBoolean(true);
        // Constants.barkRate = config.get("doggySettings", "barkRate", 10, "Default is 10, higher the number the slower
        // the dogs bark. Lower the number the faster the dogs bark.").getInt(10);
        Constants.DIRE_PARTICLES = CONFIG
            .get(CATEGORY_DOGGYSETTINGS, "direParticles", true, "Enables the particle effect on Dire Level 30 dogs.")
            .getBoolean(true);
        Constants.STARTING_ITEMS = CONFIG
            .get(
                CATEGORY_DOGGYSETTINGS,
                "isStartingItemsEnabled",
                true,
                "When enabled you will spawn with a guide, Doggy Charm and Command Emblem.")
            .getBoolean(true);
        Constants.RENDER_BLOOD = CONFIG
            .get(
                CATEGORY_DOGGYSETTINGS,
                "bloodWhenIncapacitated",
                true,
                "When enabled, Dogs will bleed while incapacitated.")
            .getBoolean(true);
        Constants.DOGGY_WINGS = CONFIG
            .get(
                CATEGORY_DOGGYSETTINGS,
                "doggyWings",
                true,
        ...
        String[] talentIds = new String[] { "bedfinder", "blackpelt", "creepersweeper", "doggydash", "fisherdog",
            "guarddog", "happyeater", "hellhound", "hunterdog", "packpuppy", "pestfighter", "pillowpaw", "poisonfang",
            "puppyeyes", "quickhealer", "rescuedog", "shepherddog", "swimmerdog", "wolfmount" };
        for (String talentId : talentIds) {
            boolean enabled = CONFIG.get(CATEGORY_TALENT, talentId, true)
                .getBoolean(true);
            if (!enabled) Constants.DISABLED_TALENTS.add(talentId);
        }
    ...
    public void onConfigurationChangedEvent(ConfigChangedEvent.OnConfigChangedEvent event) {
        if (event.modID.equals(Reference.MOD_ID)) loadConfig();
    }
}

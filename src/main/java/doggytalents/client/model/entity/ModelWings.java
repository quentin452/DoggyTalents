        ...
        if (dog.isSitting()) {
            ...
            if (!dog.onGround) {
                if (!Minecraft.getMinecraft().gameSettings.keyBindBack.getIsKeyPressed()) {
                    float c = 4.0F;
                    wingA.rotateAngleY = 1.570796F
                        - (float) ((Math.atan(Math.abs(dog.motionX * c) + Math.abs(dog.motionZ * c))));
                    wingB.rotateAngleY = -1.570796F
                        - (float) -((Math.atan(Math.abs(dog.motionX * c) + Math.abs(dog.motionZ * c))));
                } else {
                    float c2 = 0.5F;
                    wingA.rotateAngleY = 1.570796F
                        - (float) ((Math.atan(Math.abs(dog.motionX * c2) + Math.abs(dog.motionZ * c2))));
                    wingB.rotateAngleY = -1.570796F
                        - (float) -((Math.atan(Math.abs(dog.motionX * c2) + Math.abs(dog.motionZ * c2))));
                }
            }

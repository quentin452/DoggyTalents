...
public class InventoryTreatBag extends InventoryBasic {
    ...
    public boolean isItemValidForSlot(int index, ItemStack stack) {
        return stack != null && (stack.getItem() instanceof IDogTreat || stack.getItem() == ModItems.CHEW_STICK
            || stack.getItem() == Items.rotten_flesh
            || (stack.getItem() instanceof ItemFood && ((ItemFood) stack.getItem()).isWolfsFavoriteMeat()));
    }

    public void openChest() {
        if (this.itemstack.hasTagCompound() && this.itemstack.getTagCompound()
            .hasKey("inventory", 10)) {
            NBTTagList nbttaglist = this.itemstack.getTagCompound()
    ...
    public void closeChest() {
        ...
        if (!this.itemstack.hasTagCompound()) this.itemstack.setTagCompound(new NBTTagCompound());

        if (!this.itemstack.getTagCompound()
            .hasKey("inventory", 10))
            this.itemstack.getTagCompound()
                .setTag("inventory", new NBTTagCompound());

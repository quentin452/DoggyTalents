...
public class EntityAIFollowOwner extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        ...
        else if (this.dog.isSitting()) return false;
        else if (!this.dog.mode.getMode()
            .doesFollowOwner()) return false;
        else if (this.dog.aiFetchBone.isPlayingFetch()) return false;
        ...
        if (!this.dog.hasBone() && (order == 1 || order == 2)) { // Holding Sword or tool
            if (distanceAway >= 16D && distanceAway <= 100D) return false;
        } else if (!this.dog.hasBone()
            && this.dog.getDistanceSqToEntity(entitylivingbase) < (double) (this.minDist * this.minDist)) return false;
    ...
    public boolean continueExecuting() {
        return !this.petPathfinder.noPath()
            && this.dog.getDistanceSqToEntity(this.owner) > (double) (this.maxDist * this.maxDist)
            && !this.dog.isSitting();
    }
    ...
    public void startExecuting() {
        ...
        this.preShouldAvoidWater = this.dog.getNavigator()
            .getAvoidsWater();
        this.dog.getNavigator()
            .setAvoidsWater(false);
        this.oldRangeSense = this.dog.getEntityAttribute(SharedMonsterAttributes.followRange)
            .getAttributeValue();
        this.dog.getEntityAttribute(SharedMonsterAttributes.followRange)
            .setBaseValue(32.0D);
    }
    ...
    public void resetTask() {
        ...
        this.dog.getNavigator()
            .setAvoidsWater(this.preShouldAvoidWater);
        this.dog.getEntityAttribute(SharedMonsterAttributes.followRange)
            .setBaseValue(this.oldRangeSense);
    }
        ...
        if (!this.dog.isSitting()) {
            if (--this.timeToRecalcPath <= 0) {
                ...
                if (((order == 0 || order == 3) && distanceAway >= 4.0D) || this.dog.hasBone()) {
                    this.dog.getLookHelper()
                        .setLookPositionWithEntity(this.owner, 10.0F, (float) this.dog.getVerticalFaceSpeed());
                    if (!this.petPathfinder.tryMoveToEntityLiving(this.owner, this.followSpeed))
                        if (!this.dog.hasBone() && !this.dog.getLeashed()) if (distanceAway >= 144.0D)
                    ...
                    if (distanceAway < 25.0D) {
                        if (this.petPathfinder.tryMoveToXYZ(j3, dogY, k3, this.followSpeed)) this.dog.getLookHelper()
                            .setLookPosition(j3, dogY + 1, k3, 10.0F, (float) this.dog.getVerticalFaceSpeed());
                    } else if (distanceAway > 100.0D) {

                        if (!this.dog.getNavigator()
                            .tryMoveToXYZ(l3, dogY, i4, this.followSpeed)) {
                            if (!this.dog.getLeashed()) if (distanceAway >= 350.0D)
                                DogUtil.teleportDogToOwner(this.owner, this.dog, this.world, this.petPathfinder);
                        } else this.dog.getLookHelper()
                            .setLookPosition(l3, dogY + 1, i4, 10.0F, (float) this.dog.getVerticalFaceSpeed());
                    }
                }
            }
        }

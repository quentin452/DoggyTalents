...
public class EntityAIFetch extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        ...
        else if (fetchableItem == null) return false;
        else if (!this.dog.mode.isMode(EnumMode.DOCILE)) return false;
        else if (this.dog.hasBone()) return false;
        else if (this.dog.isSitting()) return false;
        else if (this.dog.isIncapacicated()) return false;
    ...
    public boolean continueExecuting() {
        return !this.petPathfinder.noPath() && this.isPlayingFetch()
            && this.dog.getDistanceSqToEntity(this.fetchableItem) < (double) (this.maxDist * this.maxDist)
            && !this.dog.isSitting();
    }
    ...
    public EntityItem getNearestFetchableItem() {
        List<Entity> list = this.world.getEntitiesWithinAABBExcludingEntity(
            this.dog,
            this.dog.boundingBox.expand(this.maxDist, this.maxDist, this.maxDist));
        for (Entity entity : list) {
            if (entity instanceof EntityItem) {
                EntityItem entityItem = (EntityItem) entity;

                if (entityItem.getEntityItem()
                    .getItem() == ModItems.THROW_BONE
    ...
    public void updateTask() {
        this.dog.getLookHelper()
            .setLookPositionWithEntity(this.fetchableItem, 10.0F, (float) this.dog.getVerticalFaceSpeed());
        ...
        if (!this.dog.isSitting()) {
            ...
            if (this.dog.getDistanceSqToEntity(this.fetchableItem) < (double) (1.5F * 1.5F) && !this.dog.hasBone()) {
                if (this.fetchableItem.isEntityAlive()) {
                    ...
                    this.dog.setBoneVariant(
                        this.fetchableItem.getEntityItem()
                            .getItemDamage() / 2);
                    this.fetchableItem = null;
                    this.dog.getNavigator()
                        .clearPathEntity();

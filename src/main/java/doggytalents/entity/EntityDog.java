...
public class EntityDog extends EntityAbstractDog {
    ...
    public void updateEntityAttributes() {
        if (this.isTamed()) this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .setBaseValue(20.0D);
        else this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .setBaseValue(8.0D);
    }
    ...
    protected void entityInit() {
        ...
        this.dataWatcher.addObject(19, -2); // Dog Cape
        this.dataWatcher.addObject(20, -2); // Dog Collar
        this.dataWatcher.addObject(21, ""); // Dog Name
        this.dataWatcher.addObject(22, ""); // Talent Data
        this.dataWatcher.addObject(23, 60); // Dog Hunger
        this.dataWatcher.addObject(24, "0:0"); // Level Data
        this.dataWatcher.addObject(26, 0); // Texture index
        this.dataWatcher.addObject(27, 0); // Dog Mode
    ...
    public void readEntityFromNBT(NBTTagCompound tagCompound) {
        ...
        if (tagCompound.hasKey("dogName")) this.setCustomNameTag(tagCompound.getString("dogName"));
    }
    ...
    public void onLivingUpdate() {
        ...
        if (Constants.IS_HUNGER_ON) {
            ...
            if (this.riddenByEntity == null
                && !this.isSitting() /**
                                      * && !this.mode.isMode(EnumMode.WANDERING) && !this.level.isDireDog() ||
                                      * worldObj.getWorldInfo().getWorldTime() % 2L == 0L
                                      **/
            ) this.hungerTick += 1;
        ...
        if (Constants.DOGS_IMMORTAL) {
            ...
            if (this.isSitting()) {
                ...
                this.regenerationTick += TalentHelper
                    .onRegenerationTick(this, this.regenerationTick - this.prevRegenerationTick);
            } else if (!this.isSitting()) this.regenerationTick = 0;
        ...
        if (this.getDogHunger() <= 0 && this.worldObj.getWorldInfo()
            .getWorldTime() % 100L == 0L && this.getHealth() > 1) {
            this.attackEntityFrom(DamageSource.generic, 1);
            // this.fleeingTick = 0;
        }

        if (this.levels.isDireDog() && Constants.DIRE_PARTICLES) for (int i = 0; i < 2; i++) worldObj.spawnParticle(
            "portal",
            this.posX + (this.rand.nextDouble() - 0.5D) * (double) this.width,
            (this.posY + rand.nextDouble() * (double) height) - 0.25D,
            posZ + (rand.nextDouble() - 0.5D) * (double) this.width,
            (this.rand.nextDouble() - 0.5D) * 2D,
            -this.rand.nextDouble(),
            (this.rand.nextDouble() - 0.5D) * 2D);

        if (this.reversionTime > 0) this.reversionTime -= 1;

        // Remove dog from players head if sneaking
        if (this.ridingEntity instanceof EntityPlayer) if (this.ridingEntity.isSneaking()) {
        ...
        if (this.coords.hasBowlPos() && this.foodBowlCheck++ > 50) {
            if (this.worldObj.blockExists(this.coords.getBowlX(), this.coords.getBowlY(), this.coords.getBowlZ()))
                if (this.worldObj.getBlock(this.coords.getBowlX(), this.coords.getBowlY(), this.coords.getBowlZ())
                    != ModBlocks.FOOD_BOWL) this.coords.resetBowlPosition();
    ...
    public void onUpdate() {
        ...
        if (this.rand.nextInt(200) == 0) this.hiyaMaster = true;
        ...
        } else this.hiyaMaster = false;

        if (this.isWolfHappy) {
            if (this.timeWolfIsHappy % 1.0F == 0.0F) this.worldObj.playSoundAtEntity(
                this,
                "mob.wolf.panting",
                this.getSoundVolume(),
    ...
    public void moveEntityWithHeading(float strafe, float forward) {
        if (this.isControllingPassengerPlayer()) {
            ...
            if (entitylivingbase instanceof EntityPlayer) {
                float f = (float) this.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
                    .getAttributeValue() * 0.5F;
    ...
    public void fall(float distance) {
        if (!TalentHelper.isImmuneToFalls(this)) super.fall(
            distance - TalentHelper.fallProtection(this) - (this.riddenByEntity instanceof EntityPlayer ? 1 : 0));
    }
    ...
    public boolean attackEntityFrom(DamageSource damageSource, float damage) {
        if (this.isEntityInvulnerable()) return false;
            ...
            // Friendly fire
            if (!this.canFriendlyFire() && entity instanceof EntityPlayer
                && (this.willObeyOthers() || this.func_152114_e((EntityPlayer) entity))) return false;

            if (!TalentHelper.attackEntityFrom(this, damageSource, damage)) return false;
    ...
    public boolean attackEntityAsMob(Entity entity) {
        if (!TalentHelper.shouldDamageMob(this, entity)) return false;

        int damage = 4 + (MathHelper.floor_double(this.effectiveLevel()) + 1) / 2;
        damage = TalentHelper.attackEntityAsMob(this, entity, damage);

        if (entity instanceof EntityZombie) ((EntityZombie) entity).setAttackTarget(this);
    ...
    public void setTamed(boolean p_70903_1_) {
        ...
        if (p_70903_1_) this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .setBaseValue(20.0D);
        else this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .setBaseValue(8.0D);
    }
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (TalentHelper.interactWithPlayer(this, player)) return true;

        if (this.isTamed()) {
            if (stack != null) {
                int foodValue = this.foodValue(stack);

                if (foodValue != 0 && this.getDogHunger() < 120
                    && this.canInteract(player)
                    && !this.isIncapacicated()) {
                    ...
                    if (!this.worldObj.isRemote) {
                        ...
                        else this.mountEntity(player);
                    }

                    if (this.aiSit != null) this.aiSit.setSitting(true);

                    return true;
                } else if (stack.getItem() == Items.stick && this.canInteract(player) && !this.isIncapacicated()) {
                    player.openGui(
                        DoggyTalents.INSTANCE,
                        CommonProxy.GUI_ID_DOGGY,
                        this.worldObj,
                        this.getEntityId(),
                        MathHelper.floor_double(this.posY),
                        MathHelper.floor_double(this.posZ));
                    return true;
                } else if (stack.getItem() == ModItems.RADIO_COLLAR && this.canInteract(player)
                    && !this.hasRadarCollar()
                    && !this.isIncapacicated()) {
                        ...
                        if (!player.capabilities.isCreativeMode) --stack.stackSize;
                        return true;
                    } else if (stack.getItem() == ModItems.WOOL_COLLAR && this.canInteract(player)
                        && !this.hasCollar()
                        && !this.isIncapacicated()) {
                            int colour = -1;

                            if (stack.hasTagCompound() && stack.getTagCompound()
                                .hasKey("collar_colour"))
                                colour = stack.getTagCompound()
                                    .getInteger("collar_colour");

                            this.setCollarData(colour);

                            if (!player.capabilities.isCreativeMode) --stack.stackSize;
                            return true;
                        } else if (stack.getItem() == ModItems.FANCY_COLLAR && this.canInteract(player)
                            && !this.hasCollar()
                            && !this.isIncapacicated()) {
                                ...
                                if (!player.capabilities.isCreativeMode) --stack.stackSize;
                                return true;
                            } else if (stack.getItem() == ModItems.CAPE && this.canInteract(player)
                                && !this.hasCape()
                                && !this.isIncapacicated()) {
                                    this.setFancyCape();
                                    if (!player.capabilities.isCreativeMode) --stack.stackSize;
                                    return true;
                                } else if (stack.getItem() == ModItems.CAPE_COLOURED && this.canInteract(player)
                                    && !this.hasCape()
                                    && !this.isIncapacicated()) {
                                        int colour = -1;
                                        ...
                                        if (stack.hasTagCompound() && stack.getTagCompound()
                                            .hasKey("cape_colour"))
                                            colour = stack.getTagCompound()
                                                .getInteger("cape_colour");

                                        this.setCapeData(colour);

                                        if (!player.capabilities.isCreativeMode) --stack.stackSize;
                                        return true;
                                    } else if (stack.getItem() == ModItems.SUNGLASSES && this.canInteract(player)
                                        && !this.hasSunglasses()
                                        && !this.isIncapacicated()) {
                                        ...
                                        if (stack.getItem() == ModItems.LEATHER_JACKET && this.canInteract(player)
                                            && !this.hasCape()
                                            && !this.isIncapacicated()) {
                                                ...
                                                && !this.isIncapacicated()) {
                                                    ...
                                                    EnumFeedBack type = treat.canGiveToDog(
                                                        player,
                                                        this,
                                                        this.levels.getLevel(),
                                                        this.levels.getDireLevel());
                                                    treat.giveTreat(type, player, stack, this);
                                                    return true;
                                                } else if (stack.getItem() == ModItems.COLLAR_SHEARS
                                                    && this.canInteract(player)) {
                                                        if (!this.worldObj.isRemote) {
                                                            if (this.hasCollar() || this.hasSunglasses()
                                                                || this.hasCape()) {
                                                                this.reversionTime = 40;
                                                                if (this.hasCollarColoured()) {
                                                                    ItemStack collarDrop = new ItemStack(
                                                                        ModItems.WOOL_COLLAR,
                                                                        1,
                                                                        0);
                                                                    if (this.isCollarColoured()) {
                                                                        collarDrop.setTagCompound(new NBTTagCompound());
                                                                        collarDrop.getTagCompound()
                                                                            .setInteger(
                                                                                "collar_colour",
                                                                                this.getCollarData());
                                                                    }
                                                                ...
                                                                if (this.hasFancyCollar()) {
                                                                    this.entityDropItem(
                                                                        new ItemStack(
                                                                            ModItems.FANCY_COLLAR,
                                                                            1,
                                                                            this.getFancyCollarIndex()),
                                                                        1);
                                                                    this.setNoCollar();
                                                                }
                                                                ...
                                                                if (this.hasLeatherJacket()) {
                                                                    this.entityDropItem(
                                                                        new ItemStack(ModItems.LEATHER_JACKET, 1, 0),
                                                                        1);
                                                                    this.setNoCape();
                                                                }
                                                                ...
                                                                if (this.hasSunglasses()) {
                                                                    this.entityDropItem(
                                                                        new ItemStack(ModItems.SUNGLASSES, 1, 0),
                                                                        1);
                                                        ...
                                                        return true;
                                                    } else if (stack.getItem() == Items.cake && this.canInteract(player)
                                                        && this.isIncapacicated()) {
                                                            if (!player.capabilities.isCreativeMode) --stack.stackSize;
                                                        ...
                                                        if (stack.getItem() == Items.dye && this.canInteract(player)) {
                                                            if (!this.hasCollarColoured()) return true;

                                                            if (this.isCollarColoured()) {
                                                                float[] rgb = EntitySheep.fleeceColorTable[BlockColored
                                                                ...
                                                                int[] aint = new int[3];
                                                                int i = 0;
                                                                int count = 2; // The number of different sources of
                                                                               // colour
                                                                ...
                                                                i = (int) ((float) i
                                                                    + Math.max(f, Math.max(f1, f2)) * 255.0F);
                                                            ...
                                                            return true;
                                                        } else if (stack.getItem() == ModItems.TREAT_BAG
                                                            && this.getDogHunger() < 120
                                                            && this.canInteract(player)) {

                                                                InventoryTreatBag treatBag = new InventoryTreatBag(
                                                                    player,
                                                                    player.inventory.currentItem,
                                                                    stack);
                                                                treatBag.openChest();

                                                                int slotIndex = DogUtil
                                                                    .getFirstSlotWithFood(this, treatBag);
                                                                if (slotIndex >= 0)
        ...
        } else if (stack != null && stack.getItem() == ModItems.COLLAR_SHEARS
    ...
    protected boolean isMovementBlocked() {
        return this.isPlayerSleeping() || super.isMovementBlocked(); // this.ridingEntity != null || this.riddenByEntity
                                                                     // instanceof EntityPlayer ||
                                                                     // super.isMovementBlocked();
    }

    @Override
    public boolean isPotionApplicable(PotionEffect potionEffect) {
        if (this.isIncapacicated()) return false;

        if (!TalentHelper.isPostionApplicable(this, potionEffect)) return false;
    ...
    public void setFire(int amount) {
        if (TalentHelper.setFire(this, amount)) super.setFire(amount);
    }
    ...
    public int foodValue(ItemStack stack) {
        ...
        if (stack.getItem() != Items.rotten_flesh && item instanceof ItemFood) {
            ...
            if (itemfood.isWolfsFavoriteMeat()) foodValue = 40;
    ...
    public int masterOrder() {
        ...
        if (player != null) {
            ...
            if (itemstack != null && (itemstack.getItem() instanceof ItemTool) && distanceAway <= 20F) order = 1;

            if (itemstack != null
                && ((itemstack.getItem() instanceof ItemSword) || (itemstack.getItem() instanceof ItemBow))) order = 2;

            if (itemstack != null && itemstack.getItem() == Items.wheat) order = 3;
        }
    ...
    public float getWagAngle(float partialTickTime, float offset) {
        float f = (this.prevTimeWolfIsHappy + (this.timeWolfIsHappy - this.prevTimeWolfIsHappy) * partialTickTime
            + offset) / 2.0F;
        if (f < 0.0F) f = 0.0F;
    ...
    public int points() {
        return this.isCreativeCollar() ? 1000
            : this.levels.getLevel() + this.levels.getDireLevel()
                + (this.levels.isDireDog() ? 15 : 0)
                + (this.getGrowingAge() < 0 ? 0 : 15);
    }
    ...
    public boolean func_142018_a(EntityLivingBase entityToAttack, EntityLivingBase owner) {
        if (TalentHelper.canAttackEntity(this, entityToAttack)) return true;
    ...
    public boolean shouldDismountInWater(Entity rider) {
        if (!TalentHelper.shouldDismountInWater(this, rider)) return false;
